...
class Datashift extends BaseModel
{

    protected $table = 'datashift';
    ...
    protected static function boot()
    {
        ...
        static::addGlobalScope('company', function (Builder $builder) {
            if (admin()) {
//                $builder->join('employees', 'datashift.employee_id', '=', 'employees.id');
//                $builder->where('employees.company_id', '=', admin()->company_id);
            }
            if (employee()) {
//                $builder->join('employees', 'datashift.employee_id', '=', 'employees.id')
//                    ->select('*','datashift.status as a_status')
//                    ->where('employees.company_id', '=', employee()->company_id);
    ...
    {
        $fullday = count(DB::select(DB::raw("select * from datashift where YEAR(date)=" . $year . "
	                                            AND MONTH(date)=" . $month . " AND datashift.status='alpha'
	                                            AND employee_id='$employeeID'")));

        $halfday = count(DB::select(DB::raw("select * from datashift where YEAR(date)=" . $year . "
			                                    AND MONTH(date)=" . $month . " AND datashift.status='hadir' AND halfDayType='yes'
    ...
    public static function datashiftCount($employeeID, $company_id)
    {
        // Calculating Datashift
        $date = date('d');
        $month = date('m');
        $year = date('Y');
        $firstDay = $year . '-' . $month . '-' . $date;

        $presentCount = Datashift::countPresentDays($month, $year, $employeeID);
    ...
    public static function absentEmployee($employeeID)
    {
        ...
        $absent = [];
        foreach (Leavetype::get() as $leave) {
            $half_day = Datashift::where('datashift.status', '=', 'hadir')->where(function ($query) {
            ...
            // Added to casual
            $half = $half_day / 2;
            $absent[$leave->leaveType] = Datashift::where('datashift.status', '=', 'hadir')->where(function ($query) {
                $query->where('application_status', '=', 'approved')
                    ->orWhere('application_status', '=', null);
            })->where('employee_id', '=', $employeeID)//For Unpaid Leaves
    ...
    public static function absentEveryEmployee()
    {
        ...
        foreach ($employees as $employee) {
            ...
            foreach (Leavetype::get() as $leave) {
                ...
                $absentess[$employee->employeeID][$leave->leaveType] = Datashift::where('datashift.status', '=', 'hadir')
                ...
                $half_day = Datashift::where('status', '=', 'hadir')
                    ->where('employee_id', '=', $employee->id)
                    ->where(function ($query) {
                        $query->where('application_status', '=', 'approved')
                            ->orWhere('application_status', '=', null);
    ...
    {
        return $query->join('employees', 'datashift.employee_id', '=', 'employees.id')
    ...
    public function getClockInAttribute($value)
    {
        ...
        // This case occurs if datashift is of prev day and user clocked in today
    ...
    public function getClockOutAttribute($value)
    {
        ...
        // This occurs if datashift is of prev day and user clocked in today
