...
class TanggalAbsensi extends BaseModel
{
    ...
    protected $table = 'tanggal_absensis';
    protected $fillable = [];
    ...
    public static function rules_single_leaves($input)
    {
        ...
        foreach ($input as $key => $val) {
            $rules_single_leaves['date.' . $key] = 'unique:tanggal_absensis,start_date,NULL,id,employeeID,' . auth()->guard('employee')->user()->employeeID;
        }
        $rules_single_leaves['date.0'] = 'required|date|unique:tanggal_absensis,start_date,NULL,id,employeeID,' . auth()->guard('employee')->user()->employeeID;
