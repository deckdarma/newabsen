...
class Setting extends BaseModel
{
    ...
    public static $rules = [
        ...
        'logo_tampilan' => 'image|mimes:jpeg,jpg,png,bmp,gif,svg|max:1000'

    ];

    protected $guarded = ['id'];
    protected $appends = ['logo_image_url', 'logo_tampilan_image_url', 'set_smtp_message', 'favicon_image_url'];
    ...
    public function getLogoImageUrlAttribute($size = 150, $d = 'mm')
    {
	    ...
	    public function getLogoTampilanImageUrlAttribute($size = 150, $d = 'mm')
    {
        if (is_null($this->logo_tampilan)) {
            return $url = asset('assets/admin/layout/img/hrm-logo-full.png');
        }

        if (strpos($this->logo_tampilan, 'https://') !== false) {
            return $image = str_replace('type=normal', 'type=large', $this->logo_tampilan);
        }

        return asset_url('setting/logo/' . $this->logo_tampilan);
    }
