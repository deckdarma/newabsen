...
class Attendance2 extends BaseModel
{
...
//                    ->where('employees.company_id', '=', employee()->company_id);
    ...
    public static function countPresentDays($month, $year, $employeeID)
    {
	    ...
	    public static function countAbsentDays($month, $year, $employeeID)
    {
        $fullday = count(DB::select(DB::raw("select * from attendance where YEAR(date)=" . $year . "
	                                            AND MONTH(date)=" . $month . " AND attendance.status='absent'
	                                            AND employee_id='$employeeID'")));
    ...
    public static function attendanceCount($employeeID, $company_id)
    {
        ...
        $holiday_count = count(DB::select(DB::raw("SELECT * FROM holidays WHERE MONTH(date)=" . $month . " AND YEAR(date)=" . $year . " ")));
