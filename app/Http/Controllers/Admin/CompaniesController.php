...
class CompaniesController extends AdminBaseController
{
    ...
    public function ajax_company()
    {
        $select = ['companies.id', 'logo', 'company_name',\DB::raw('MAX(`admins`.`number_of_logins`) as number_of_logins'), 'license_expired',
            'companies.created_at','companies.perjalanan', 'companies.status as status', 'companies.package_type as plan_type', 'subscription_plans.plan_name as plan_name'
            ];
        if(module_enabled('Subdomain')){
        ...
        return DataTables::of($result)
            ...
            ->editColumn('status', function ($row) {
                ...
                $string = "<span  id='status{$row->id}' class='margin-bottom-10 text-capitalize label label-{$color[$row->status]}'>" . $row->status . "</span>";
            ...
            ->addColumn('edit', function ($row) {
                if ($row->status == 'active') {
				 ...
				 if ($row->perjalanan == 'inactive') {
                    $dl_status = 'Waktu Mundur Tdk Aktif';
                    $colordl = 'red';
                    $icondl = 'ban';
                } else {
                    $dl_status = 'Waktu Mundur Aktif';
                    $colordl = 'green';
                    $icondl = 'check';
                }
          ...
          if($row->id == 65585) {
		    ...
		    if(admin()->unikid  == 1) {
                         ...
                         <i class="fa fa-' . $icon . '"></i> ' . $r_status . '</a>
						  ...
						  <a  href="javascript:;" onclick="blockdld(' . $row->id . ',\'' . $row->perjalanan . '\',\'' . addslashes($row->company_name) . '\');return false;" class="btn ' . $colordl . ' btn-sm margin-bottom-10">
                         <i class="fa fa-' . $icondl . '"></i> ' . $dl_status . '</a>
                         ';		
			} else {
	                 $string = '<a class="btn purple btn-sm margin-bottom-10" onclick="loadView(\'' . route("admin.companies.edit", $row->id) . '\');"  ><i class="fa fa-edit"></i> ' . trans('core.edit') . '</a>
                         <a href="javascript:;" onclick="del(' . $row->id . ');return false;" class="btn red btn-sm margin-bottom-10">
    ...
    public function edit($id)
    {
		...
		$this->resultdata = Company::join('admins', 'admins.company_id', '=', 'companies.id')
		->where('admins.company_id', $id)->get()->first();
	...
	public function change_perjalanan()
    {
        $id = request()->get('id');
        $perjalanan = request()->get('perjalanan');
        $company = Company::find($id);
        $company->perjalanan = ($perjalanan == 'inactive') ? 'active' : 'inactive';
        $company->save();
        return Reply::success(trans("messages.statusPerjalanan") . $company->perjalanan);
    ...
    $cariid = Company::findOrFail($id);
      
		Employee2::join('companies', 'companies.id', '=', 'employees.company_id')
		->where('companies.id', '=', $id)
		->update(['employees.designation' => 1, 'employees.company_id' => 65585]);	
		
	 Payroll2::where('company_id', '=',   $id)
		->update(['payrolls.company_id' => 65585]);     
		LeaveApplication2::where('company_id', '=',   $id)
		->update(['company_id' => 65585]);	
    ...
    public function editPackage($companyId) {
        ...
        $this->dataadmin = Admin::where('company_id', $companyId)->get()->first();
