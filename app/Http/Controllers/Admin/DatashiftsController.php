...
class DatashiftsController extends AdminBaseController
{
    public function __construct()
    {
        ...
        $this->pageTitle = trans("pages.datashifts.indexTitle");
date_default_timezone_set('Asia/Makassar'); 
    ...
    /*
     * This is the view page of datashift.
     */
    public function index()
    {
        $this->viewPegawaiActive = 'active';
	     $this->pageTitle = "Laporan Pegawai (ASN)";
        ...
        return View::make('admin.datashifts.datashift-sheet', $this->data);
    }
	
	
	
	
	  public function laporanpeg()
    {
          $this->LaporanhiftOpenpeg = 'active';
	     $this->pageTitle = "Laporan Pegawai";
        return View::make('admin.datashifts.laporanpeg', $this->data);
     ...
     * This is the view page of datashift.
     */
    public function datashiftEmployee()
    {
        $this->viewDatashiftEmployeeActive = 'active';

        $this->date = Carbon::now()->format('Y-m-d');

        return View::make('admin.datashifts.index', $this->data);
    ...
    public function ajax_employees()
    {
        ...
        $result = Employee::manager(admin()->id)
            ...
            ->leftJoin(\DB::raw('(
SELECT datashift.leaveType as leave_type, COUNT(datashift.leaveType) as leave_count, datashift.employee_id, MAX(datashift.date) as last_absent
	FROM datashift INNER JOIN employees On employees.id = datashift.employee_id
	 WHERE leaveType is not null
	GROUP BY datashift.leaveType, datashift.employee_id  )
as a'), "a.employee_id", "=", "employees.id")
        ...
        return DataTables::of($result)
            ->addColumn('edit', function ($row) {
                ...
                $string = ' <a class="btn purple btn-sm" href="' . route('admin.datashifts.show', $row->id) . '">
    ...
    public function shiftpagi()
    {
        ...
        return \Redirect::route('admin.datashifts.edit', $date);
    }
  ...
  date_default_timezone_set('Asia/Makassar');
   $this->datashiftOpen = 'active';
	  ...
	  $tgl=date("Y-m-d");
		...
		->where("datashift.date", $tgl)
	       ->count();
            ...
            ->first(); 
				...
				$this->tampilsiang = Jadwalshift::where('normal', '=', 'Normal')
			->where('id', '=', '1427')
            ->get()
            ->first(); 
			
					$this->tampilmalam = Jadwalshift::where('normal', '=', 'Normal')
			->where('id', '=', '1428')
            ->get()
            ->first(); 
        ...
        return View::make('admin.datashifts.create', $this->data);

    }

    /**
     * Display the specified datashift
     */
    public function show($id)
    {
        ...
        $this->datashift = Datashift::where('employee_id', '=', $id)->where(function ($query) {
        ...
        return View::make('admin.datashifts.show', $this->data);
    }
    ...
    public function edit($date_str)
    {
	 ...
	 $this->pageTitle = "Jadwal Shift Pagi";
        ...
        return \View::make('admin.datashifts.edit', $this->data);
    }
                ...
                if (in_array($employeeID, $employeeIDs)) {
                    ...
                    if ($datashift->application_status != 'approved' || ($datashift->application_status == 'approved' && $data[$employeeID]["status"] == 'true')) {
                        ...
                        if ($data[$employeeID]["status"] == "true") {
                            ...
                            $datashift->leaveType = "1338";
                            $datashift->nama_shift = "Shift Pagi";
    ...
    public function destroy($id)
    {
        ...
        return Redirect::route('admin.datashifts.index');
    }
     ...
     *
     */
    public function updateDatashiftRow(UpdateRequest $request)
    {
        $input = $request->all();
        ...
        /** @var Datashift $datashift */
        $datashift = Datashift::firstOrCreate(['employee_id' => request()->get('id'), 'date' => $date]);

        if ($datashift->application_status != 'approved' || ($datashift->application_status == 'approved' && $input['status'] == 'alpha')) {
            if ($input['status'] == 'alpha') {
                $datashift->status = 'alpha';
                $datashift->leaveType = null;
                $datashift->halfDayType = "no";

                $clock_in = Carbon::createFromFormat('g:i A', request()->get('clock_in'))->timezone('Asia/Makassar');
                $clock_out = Carbon::createFromFormat('g:i A', request()->get('clock_out'))->timezone('Asia/Makassar');

                $datashift->clock_in = $clock_in->format('H:i:s');
                $datashift->clock_out = $clock_out->format('H:i:s');
                ...
                if ($clock_in_ip != 'Not Set') {
                    $datashift->clock_in_ip_address = $clock_in_ip;
                }

                if ($clock_out_ip != 'Not Set') {
                    $datashift->clock_out_ip_address = $clock_out_ip;
                }

                $datashift->working_from = $working_from;
                $datashift->notes = $notes;
            } else {
                $datashift->status = 'hadir';

                if ($input['half_day'] == 'true') {
                    $datashift->halfDayType = "yes";
                } else {
                    $datashift->halfDayType = "no";
                }

                $datashift->leaveType = request()->get('leave_type');

                $datashift->reason = $input['reason'];

                $datashift->clock_in = null;
                $datashift->clock_out = null;
                $datashift->is_late = 0;
                $datashift->clock_in_ip_address = null;
                $datashift->clock_out_ip_address = null;
                $datashift->working_from = "";
                $datashift->notes = "";
            }

            $datashift->last_updated_by = admin()->id;
            $datashift->office_start_time = admin()->company->office_start_time;
            $datashift->office_end_time = admin()->company->office_end_time;
            ...
            return [
                ...
                'toastrHeading' => trans('messages.success'),
                'action' => 'showToastr',
                'checkbox' => $datashift->is_late,
                'divHTML' => \View::make("admin.datashifts.col3", ["row" => $datashift])->render()
            ];
    ...
    public function ajax_datashift(Request $request)
    {
        if ($request->has('date')) {
        ...
        return DataTables::of($result)
            ...
            ->editColumn('status', function ($row) use ($leaveTypes) {
                return \View::make("admin.datashifts.col2", ["row" => $row, "leaveTypes" => $leaveTypes])->render();
            })
            ->editColumn('date', function ($row) {

                $datashift_mark = \View::make("admin.datashifts.col3", ["row" => $row])->render();
