...
class LeaveApplicationsController extends AdminBaseController
{
    ...
    {
$stale_posts = LeaveApplication::where('applied_on', '<', Carbon::now()->subDays(2))
...
foreach ($stale_posts as $post) {
    ...
    public function ajaxApplications()
    {
        ...
        return DataTables::of($result)->editColumn('start_date', function ($row) {
            ...
            $leave =  ''.$row->namaketerangan.' ('.$row->singkatan.')' ;

            if ($row->halfDayType == "yes") {
                $leave .= '<br/><span class="label label-info label-sm">Setengah Hari</span>';
            }
            ...
            if ($row->application_status == 'pending') {
				         ...
				         if ($row->waktumundur == '1') {
                $string = '
                         <div class="btn-group">
						 <button class="btn green btn-sm margin-bottom-5" data-toggle="modal" href="#static_approve" onclick="show_approve(' . $row->id . ');return false;">Terima</button>

						<a  class="btn purple btn-sm margin-bottom-5"  onclick="showEdit_dl(' . $row->id . ',\'' . addslashes($row->leaveType) . '\',\'' . $row->num_of_leave . '\')"><i class="fa fa-edit"></i> <span class="hidden-sm hidden-xs">Edit</span></a>
    ...
    public function show($id)
    {
        ...
        return View::make('admin.leave_applications.show', compact('leavetypes'), $this->data);
    }
