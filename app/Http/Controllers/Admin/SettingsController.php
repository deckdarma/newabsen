...
class SettingsController extends AdminBaseController
{
  ...
  public function mesintarik()
    {
        return View::make('admin.settings.mesintarik', $this->data);
    }

	public function tampilan()
    {
        return View::make('admin.settings.tampilan', $this->data);
    }
    ...
    public function update(UpdateRequest $request, $id)
    {
        ...
        if ($request->type != 'stripeSetting' && $request->type != 'smtpSetting') {
            ...
            $data['tarikdata'] = $request->has('tarikdata') && request()->get('tarikdata') == 'on' ? 1 : 0;
        }
...
//            return Reply::redirect(route('admin.settings.edit', 'settings'));
		        ...
		        if ($request->hasFile('logo_tampilan')) {
            $file = new Files();
            $filename = $file->upload($request->file('logo_tampilan'), 'setting/logo', null, 200, false);
            $setting->logo_tampilan = $filename;
            $setting->save();
//            return Reply::redirect(route('admin.settings.edit', 'settings'));
        }
