...
//            ->whereDate('companies.created_at', '<', 'last_login')
		...
		$this->jumlah_asn =	DB::table('employees')
		->where("statusmupeg", "ASN")
			->where("status", "active")
			->where("designation", "!=", NULL)
		->count();
				
		$this->jumlah_phl =	DB::table('employees')
		...
		->count();
				...
				$this->jumlah_ip =	DB::table('fingerprint_machines')
    ...
    public function createEarningReport($year)
    {
        ...
        $earnings = DB::select(DB::raw("SELECT sum(jumlah_bersih_keseluruhan) as sum, m.months,u.status,employees.company_id
...
ORDER BY m.months;"));
		...
		foreach ($earnings as $ex) {
            $earningVal[$ex->months] = isset($ex->sum) ? $ex->sum : "''";
        }
        ksort($earningVal);
