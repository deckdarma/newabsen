...
class AdminDashboardController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        //Expense Calculation
        $expense = DB::select(DB::raw("SELECT sum(jumlah_bersih_keseluruhan) as sum, m.months,u.status,employees.company_id
    FROM `payrolls` u JOIN employees on employees.id=u.employee_id
...
ON m.months = MONTH(u.pay_date)
WHERE u.status = 'paid' OR u.status is null
GROUP BY m.months
ORDER BY m.months;"));
        ...
        $this->jumlah_phl =  Employee::manager()
			->where("statusmupeg", "PHL")
					->where("status", "active")
		->count();
		
	    $this->jumlah_asn =  Employee::manager()
...
//        print_r($this->awards);die;
    ...
    public function ajax_load_calender()
    {
        ...
        if ($start_month == 12) {
            $employees = Employee::select('employees.id', 'employees.full_name as title', 'employees.datenuls as date', DB::raw('\'birthday\' as type'))
                ->where(DB::raw('month(datenuls)'), '<=', $start_month)
                ->where(DB::raw('month(datenuls)'), '<', $end_month)->get()->toArray();
