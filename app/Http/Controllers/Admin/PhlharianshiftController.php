...
class PhlharianshiftController extends AdminBaseController
{
...
date_default_timezone_set('Asia/Makassar'); 
  ...
  public function lihatdata()
    {
  date_default_timezone_set('Asia/Makassar');
  	        $this->phldatashifMenuActive = 'active';
	     $this->pageTitle = "Laporan Harian Shift";
	  ...
	  $tgl=date("Y-m-d");
        ...
        return View::make('admin.phlharianshift.lihatdata', $this->data);
...
as a'), "a.employee_id", "=", "employees.id")
        ...
        return DataTables::of($result)
            ->addColumn('edit', function ($row) {
                ...
                $string = ' <a class="btn purple btn-sm" href="' . route('admin.phlharianshift.show', $row->id) . '">
    ...
    public function phldata()
    {
        ...
        return \Redirect::route('admin.phlharianshift.edit', $date);
    }
   ...
   public function edit($date_str)
    {
		...
		$this->phldatashifMenuActive = 'active';
		   $this->pageTitle = "Laporan Harian PHL Shift";
        ...
        $attendanceArray = [];
			...
			$this->dataabsenlihatss = Employee::manager(admin()->id)
			...
			"employees.id as idemp", 
		
			"datashift.employee_id as idshift", 
			"datashift.status as statusss", 
        ...
        return \View::make('admin.phlharianshift.edit',  $this->data);
    }
    ...
    public function destroy($id)
    {
        ...
        return Redirect::route('admin.phlharianshift.index');
    }
...
$result = Employee::manager(admin()->id)
            ->leftJoin("attendance", function ($query) use ($date) {
			 ...
			 ->leftJoin('jadwalshifts', 'datashift.leaveType', '=', 'jadwalshifts.id', )
		
		->where('datashift.status', '=', 'hadir')

            ->select('employees.full_name as full_name',
			'leavetypes.leaveType as keterangan',
			"attendance.date as dateshifs", 
			"datashift.nama_shift as nama_shift", 
			"datashift.leaveType as iddateshif", 
...
"attendance.status as status", 
        ...
        return DataTables::of($result)
            ...
            ->editColumn('status', function ($row) use ($leaveTypes) {
                return \View::make("admin.phlharianshift.col2", ["row" => $row, "leaveTypes" => $leaveTypes])->render();
            })
            ->editColumn('date', function ($row) {

                $attendance_mark = \View::make("admin.phlharianshift.col3", ["row" => $row])->render();
            ...
            ->editColumn('clock_in', function ($row) {
                ...
                return \View::make("admin.phlharianshift.col4", [
