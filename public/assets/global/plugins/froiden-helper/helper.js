    ...
    $.easyAjax = function(options) {
        ...
        if (typeof opt.error != "function") {
            opt.error = function(jqXHR, textStatus, errorThrown) {
                var msg = "Terjadi kesalahan sisi server. Silakan coba lagi setelah beberapa saat.";
                ...
                if (response.status == "fail") {
                    ...
                    if (typeof response.errors != "undefined") {
                        ...
                        for (var i = 0; i < keys.length; i++) {
                            ...
                            console.log(key, ele, 'in error');

                            var grp = ele.closest(".form-group");
                            $(grp).find(".help-block").remove();
                            $(grp).find("div:first").append('<div class="help-block">' + response.errors[keys[i]] + '</div>');
