...
<div id="id-elemen-yang-ingin-di-print"> 
						...
						<div style="text-align: center;font-weight: bold;font-size: 25px;">	LAPORAN BULANAN PEGAWAI SHIFT (ASN/PHL)        </div>
...
<div class="penjelasan">Penjealsan Kode: <span class="hadir">HD : Hadir</span>  / <span class="keterangan">DK : Dengan Keterangan</span>   /  <span class="tanpa">TK : Tanpa Keterangan</span> /  <span class="pulang">SP : Sudah Pulang</span></div>
...
$date_create = $datatahun . '-' . $bulanini . '-'.$datahari;
 ...
 ?>
<?php
$carilibur_data = count(DB::select(DB::raw("SELECT * FROM datashift WHERE  datashift.status='hadir' AND   DAY(date)=" . $datahari . " AND MONTH(date)=" . $databulan . "  AND YEAR(date)=" . $datatahun . " ")));
$jumlahharikerja++;
...
?>	  
                @foreach ($staff->attendance as $att)
                        ...
                        @if ($date_create == $date_to_check) 
													                     <?php
$hitungcounthadir= count(DB::select(DB::raw("SELECT datashift.date FROM attendance LEFT JOIN datashift ON attendance.date = datashift.date WHERE attendance.status ='present' AND  datashift.date='".$date_to_check."' AND  datashift.status='hadir'   AND attendance.employee_id=" . $att->employee_id)));
$hitungcountket = count(DB::select(DB::raw("SELECT datashift.date FROM attendance LEFT JOIN datashift ON attendance.date = datashift.date WHERE attendance.status ='absent' AND attendance.application_status ='approved' AND  datashift.status='hadir' AND  datashift.date='".$date_to_check."'  AND attendance.employee_id=" . $att->employee_id)));
...
@endif
		...
		<td class="datatotal">{{ $hitungcountcounthadir }}</td>
		<td class="datatotal">{{ $hitungcountcountket }}</td>
		
		<?php $totaltanpaket = $jumlahharikerja-$hitungcountcounthadir-$hitungcountcountket; ?>
