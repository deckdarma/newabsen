...
Pengaturan Periode  Absensi
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb breadcrumb">
            ...
            <li>
                <span class="active">Pengaturan Periode  Absensi</span>
            </li>
...
{!! HTML::script('front_assets/plugins/scrollbar/src/perfect-scrollbar.js') !!}
    ...
    <script>
        ...
        var table = $('#applications').dataTable({
            ...
            "aaSorting": [[0, "desc"]],
            {!! $datatabble_lang !!}
            "ajax": "{{ URL::route('admin.tanggal_absensis') }}",
            "columns": [
                {data: 'id', name: 'id'},
        ...
        function del(id) {
            ...
            $("#deleteModal").find('#info').html("@lang("messages.tanggalAbsensiDeleteConfirm")");
            $('#deleteModal').find("#delete").off().click(function () {
                var url = "{{ route('admin.tanggal_absensis.destroy',':id') }}";
                ...
                $.ajax({
                    ...
                    if (response.success == "deleted") {
                        ...
                        $('#row' + id).fadeOut(500);
                        table._fnDraw();
                        showToastrMessage("@lang("messages.tanggalAbsensiDeleteMessage")", '{{__('core.success')}}', 'success');
                    }
                });
            })
        }
        ...
        function show_application(id) {
            var url = "{!!  route('admin.tanggal_absensis.show',':id')  !!}";
            url = url.replace(':id', id);
            $.ajaxModal('#showModal', url);
        }

        function showAdd() {
            var url = "{{ route('admin.tampilabsen.create') }}";
            $.ajaxModal('#showModal', url);
      ...
      function showEdit(id) {
            var url = "{{ route('admin.tampilabsen.edit',':id') }}";
            url = url.replace(':id', id);
            $.ajaxModal('#showModal', url);
            ...
            if (typeof id != 'undefined') {
                var url = "{{ route('admin.tampilabsen.update',':id') }}";
                url = url.replace(':id', id);
            } else {
                url = "{{route('admin.tampilabsen.store')}}";
            }
            $.easyAjax({
                type: 'POST',
                url: url,
