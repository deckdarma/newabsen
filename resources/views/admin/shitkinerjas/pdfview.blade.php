...
<table class="emp">
    ...
    <tr>
        <td colspan="3" style="text-align: center; font-size: 18px;"><strong>@lang("core.payslip") <br>
                @lang("core.salaryMonth"): {{ date('F', mktime(0, 0, 0, $shitkinerja->month, 10))}}, {{$shitkinerja->year}}
            </strong></td>
    </tr>
    <tr>
        <td><strong>@lang("core.employeeID"):</strong> {{ $shitkinerja->employee->employeeID }} </td>
        <td><strong>@lang("core.name"):</strong> {{$shitkinerja->employee->full_name}}</td>
        <td><strong>@lang("core.payslipNumber"):</strong> {{$payslip_num}}</td>
    </tr>

    <tr>
        <td><strong>@lang("core.department"):</strong> {{ $shitkinerja->employee->getDesignation->department->name}}</td>
        <td><strong>@lang("core.designation"):</strong> {{ $shitkinerja->employee->getDesignation->designation}}</td>
        <td><strong>@lang("core.joining_date")
                :</strong> {!! date('d-M,Y',strtotime($shitkinerja->employee->joining_date)) !!}</td>
    </tr>
    </tbody>
</table>
    ...
    <tr>
        ...
        <td>
            ...
            <table class="payment_details">
                ...
                <tr>
                    ...
                    <td> {{$employee->company->currency}} {{number_format($shitkinerja->basic, 2)}}</td>
                </tr>
                <tr>
                    <td>@lang("core.hourlyPayment")</td>

                    <td> {{$employee->company->currency}} {{number_format($shitkinerja->overtime_pay, 2)}} </td>
                </tr>
                <tr>
                    <td>@lang("core.expenseClaim")</td>
                    <td> {{$employee->company->currency}} {{number_format($shitkinerja->expense, 2)}} </td>
                </tr>
                @foreach(json_decode($shitkinerja->allowances) as $index=>$value)
                    <tr>
                        <td> {{ $index }}</td>
        ...
        <td>
            <table class="payment_details">
                ...
                @foreach(json_decode($shitkinerja->deductions) as $index=>$value)
        ...
        <td>
            <table class="payment_details">
                ...
                <tr>
                    ...
                    <td> {{$employee->company->currency}} {{number_format($shitkinerja->total_allowance, 2)}} </td>
                </tr>

                <tr>
                    <td>@lang("core.totalDeductions")</td>

                    <td> {{$employee->company->currency}} {{number_format($shitkinerja->total_deduction, 2)}}</td>
                </tr>
                <tr class="print_bg">
                    <td><b>@lang("core.netSalary")</b></td>

                    <td>  {{$employee->company->currency}} {{number_format($shitkinerja->net_salary, 2)}}</td>
                </tr>
            </table>
