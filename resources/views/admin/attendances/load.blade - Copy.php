...
<div id="id-elemen-yang-ingin-di-print"> 
						...
						<div style="text-align: center;font-weight: bold;font-size: 25px;">	LAPORAN BULANAN        </div>
						<div style="text-align: center;font-weight: bold;font-size: 20px;text-transform: uppercase;margin-top: -5px;">@foreach ($namadinas as $nama) {{ $nama->company_name }} @endforeach</div>
						<div style="text-align: center;font-weight: bold;font-size: 15px;text-transform: uppercase;margin-top: -5px;margin-bottom:20px;">BULAN 
...
@foreach ($employeeAttendence as $attendance)
    @foreach ($attendance as $staff)
     <?php $totalstaff++;    ?>
		 <tr>
            <td style="width:180px;">{{ $staff->full_name }}<br>{{ $staff->employeeID }}</td>
               <?php
			   $totalPresentCount = 0; 
