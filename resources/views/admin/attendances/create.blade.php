...
@if ($cekpegawai != 0)		          
	...
	@if(isset($periode_tangal->date))
		@if ($tampilkanhari == "friday") 
			Jam Masuk {{ $carinamahari }} {{ $periode_tangal->jam_masuk_jumat}}	/ Jam Pulang {{ $carinamahari }} 	{{ $periode_tangal->jam_pulang_jumat}}	 
							
			@else  
			Jam Masuk {{ $carinamahari }} {{ $periode_tangal->jam_masuk}}	/ Jam Pulang {{ $carinamahari }} 	{{ $periode_tangal->jam_pulang}}		
			@endif
		 @else
			@if ($tampilkanhari == "friday") 
			Jam Masuk {{ $carinamahari }} {{ $normal_tangal->jam_masuk_jumat}}	/ Jam Pulang {{ $carinamahari }} 	{{ $normal_tangal->jam_pulang_jumat}}	 
							
			@else  
			Jam Masuk {{ $carinamahari }} {{ $normal_tangal->jam_masuk}}	/ Jam Pulang {{ $carinamahari }} 	{{ $normal_tangal->jam_pulang}}		
			@endif
        @endif                    </div>
                            </div>
<div class="col-md-6">
...
<div class="col-md-4"> <div class="row"><h5 style="padding-top:10px;    margin: 0px;    padding-bottom: 10px;font-weight: bold;">PRESENTASI HADIR: <span>{{ $prosentaseasn }}%</span></h5> </div></div>
	      	   ...
	      	   <a style="width:100%" href="{{ route('admin.attendance.asndata') }}" class="btn  green">
...
<div class="col-md-4"> <div class="row"><h5 style="padding-top:10px;    margin: 0px;    padding-bottom: 10px;font-weight: bold;">PRESENTASI HADIR: <span>{{ $prosentasephl }}%</span></h5> </div></div>
	          ...
	          <a style="width:100%" href="{{ route('admin.laporanphl.phldata') }}"  class="btn purple">
