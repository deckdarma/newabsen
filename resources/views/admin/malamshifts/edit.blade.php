...
@section('mainarea')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb breadcrumb">
            ...
            <li>
                <span class="active">Jadwal Shift Malam</span>
            </li>
        </ul>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="portlet light bordered">
                <div class="portlet-body form">
                        ...
                        <div class="table-toolbar margin-top-15">
                            <div class="row">
                                <div class="col-md-4">
                                    {!! Form::open(['route'=>["admin.malamshifts.create"], 'method' => 'GET', 'class' => "form-inline", 'id' => "new_date"]) !!}
                                    <div class="btn-group">
                                        <div class="input-group input-medium date date-picker "
                                             data-date-viewmode="years" id="date_change">
                                            <input type="text" class="form-control " name="date"
                                                   placeholder="@lang("core.selectDate")"
                                                   readonly id="malamshift_date" value="{{ $date->format('d-m-Y') }}">
                        ...
                        @else
                            {!! Form::open(['route' => ["admin.malamshifts.update", $date->format("Y-m-d")], 'class'=>'form-horizontal ajax_form', 'method'=>'PATCH']) !!}
 	     ...
 	     <div class="text-left">
                     <input type="hidden" id="malamshiftDetails"
                                               name="malamshiftDetails[]">
                                        <button type="button" id="update_malamshift" class="btn green"
                                                onclick="ajaxUpdateDatashift()"><i class="fa fa-plus"></i>  Tambahkan Shift Malam <span
...
@section('footerjs')
    ...
    <script>
        ...
        function loadDataTable() {
            table = $('#malamshiftTable').dataTable({

                "bProcessing": false,
                "bServerSide": true,
                "ajax": {
                    "url": "{{ URL::route("admin.malamshift.ajax_malamshift") }}",
                    "data": function (d) {
                        d.date = $('#malamshift_date').val();
                    }
                },
                "aaSorting": [[0, "desc"]],
                    "autoWidth": true,
...
//		 bInfo : false
        ...
        $('#date_change').datepickerabsen({
            ...
            var url = "{{ route("admin.malamshifts.edit", "#id") }}";
            var date = moment($('#malamshift_date').val(), 'DD-MM-YYYY').format('YYYY-MM-DD');
        ...
        function ajaxUpdateDatashift() {
            ...
            var data = JSON.stringify(malamshiftData);

            var date = $('#malamshift_date').val();
            var url = "{{ route("admin.malamshifts.update", "#id") }}";
            url = url.replace("#id", date);
            ...
            $.ajax({
                ...
                method: 'POST',
                beforeSend: function () {
                    $('#update_malamshift').attr("disabled", true);
                },
                success: function (response) {
                    $('#update_malamshift').attr("disabled", false);
                    showResponseMessage(response, 'error');
                    var route = "{{ route("admin.malamshifts.edit", "#id") }}";
        ...
        function malamshiftRow(id) {
            ...
            $.ajax({
                type: "POST",
                url: "{!! route('admin.malamshift.update.row') !!}",
                data: {
                    "id": id,
                    "status": status,
