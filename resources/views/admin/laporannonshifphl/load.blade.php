...
<div id="id-elemen-yang-ingin-di-print"> 
						...
						<div style="text-align: center;font-weight: bold;font-size: 20px;text-transform: uppercase;margin-top: -5px;">{{ $loggedAdmin->company->company_name }} </div>
													<div style="text-align: center;font-weight: bold;font-size: 15px;text-transform: uppercase;margin-top: -3px;">NAMA : {{ $namapegawai }} </div>
...
<?php
$carilibur = count(DB::select(DB::raw("SELECT * FROM liburshifts WHERE  DAY(date)=" . $datahari . " AND MONTH(date)=" . $databulan . "  AND YEAR(date)=" . $datatahun . " ")));
...
<!-- STAR PERIODE --> @if($tanggalperiode ==  $datatanggal)

@include('admin.laporannonshifphl.data_periode')
<!-- ELSE PERIODE -->  @else

@include('admin.laporannonshifphl.data_normal')	
<!-- END PERIODE --> @endif
...
{{ $data->jumlah_prestasi_kehadiran }}
</td>

<td style="vertical-align: middle;">
{{ $data->jumlah_prestasi_kinerja }}
</td>

<td style="vertical-align: middle;">
{{ $data->total_prestasi_kinerja }}
</td>

<td style="vertical-align: middle;">
{{ $data->total_bobot_kinerja }}
</td>
