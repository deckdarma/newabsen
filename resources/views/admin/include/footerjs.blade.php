@php  $settingp = \App\Models\Setting::query() ->first(); @endphp                                 

  <div  style="  
    padding: 15px;
    font-size: 13px;
    background: #062b58;
    color: #fff;"> {{ $settingp->pengelola }} <div style="float: right;margin-right:35px;">Programmer Oleh : Fadli Dg Malewa</div>
	</div>
...
{{--tampilan absen--}}
{!! Form::open(['url'=>'','id'=>'show_approve_tampilan','method'=>'PATCH']) !!}
...
{{--tampilan absen--}}
{!! Form::open(['url'=>'','id'=>'show_approve_tampilan_shift','method'=>'PATCH']) !!}
...
{{--Reject--}}
{!! Form::open(['url'=>'','id'=>'show_reject_tampilan_shift','method'=>'PATCH']) !!}
...
{{--Reject--}}
{!! Form::open(['url'=>'','id'=>'show_reject_tampilan','method'=>'PATCH']) !!}
...
@if($loggedAdmin->company->status =='inactive')
	<script type="text/javascript">

    window.location.replace("{{ URL::route('admin.logout') }}");
</script>
@endif
   @else
	<STYLE>
...
//    App.unblockUI({target: ".page-content"});
    ...
    function show_approve_tampilan(id) {
        $('#showModal').modal('hide');
        $('#show_approve_tampilan').attr('action', "{{  route('admin.tanggal_absensis.show','')  }}/" + id);
    }

    function show_approve_tampilan_shift(id) {
        $('#showModal').modal('hide');
        $('#show_approve_tampilan_shift').attr('action', "{{  route('admin.tanggal_normalshifts.show','')  }}/" + id);
    }



   function LihatTampilanAPP() {
        $.ajaxModal('#Kunjungimesin', '{{route('admin.settings.tampilan')}}');
    }
	...
	function show_reject_tampilan(id) {
        $('#showModal').modal('hide');
        $('#show_reject_tampilan').attr('action', "{{  route('admin.tanggal_absensis.show','')  }}/" + id);
    }
	
		function show_reject_tampilan_shift(id) {
        $('#showModal').modal('hide');
        $('#show_reject_tampilan_shift').attr('action', "{{  route('admin.tanggal_normalshifts.show','')  }}/" + id);
    }
