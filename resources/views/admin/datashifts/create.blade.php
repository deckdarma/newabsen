...
<div class="col-md-12"> <div class="row"><h5  style="padding-top:10px;    margin: 0px;    padding-bottom: 10px;">Jadwal Pegawai yang masuk pada Shift Pagi tanggal : {{ date('d M Y') }} Sebanyak: <span style="font-weight:bold;">{{ $shiftpagi  }} org</span></h5> </div></div>
	      	   ...
	      	   <a style="width:100%" onclick="loadView('{{ route('admin.datashift.shiftpagi') }}')" class="btn  blue">
...
">HARI KERJA <b>{{ $carinamahari }}</b> : 
@if ($tampilkanharidata22 == "friday")
{{ $tampilsiang->jam_masuk_jumat }} s/d {{ $tampilsiang->jam_pulang_jumat }}
@else
{{ $tampilsiang->jam_masuk }} s/d {{ $tampilsiang->jam_pulang }}	
@endif
<BR>
<div class="col-md-12"> <div class="row"><h5  style="padding-top:10px;    margin: 0px;    padding-bottom: 10px;">Jadwal Pegawai yang masuk pada Shift Sore tanggal : {{ date('d M Y') }} Sebanyak: <span style="font-weight:bold;">{{ $shiftsiang  }} org</span></h5> </div></div>
</div>
	      	   <a style="width:100%" onclick="loadView('{{ route('admin.siangshift.shiftsiang') }}')" class="btn  red">
                  INPUT JADWAL TGL: {{ date('d M Y') }}
                </a>
...
">HARI KERJA <b>{{ $carinamahari }}</b> : 
@if ($tampilkanharidata22 == "friday")
{{ $tampilmalam->jam_masuk_jumat }} s/d {{ $tampilmalam->jam_pulang_jumat }}
@else
{{ $tampilmalam->jam_masuk }} s/d {{ $tampilmalam->jam_pulang }}	
@endif
<BR>
<div class="col-md-12"> <div class="row"><h5  style="padding-top:10px;    margin: 0px;    padding-bottom: 10px;">Jadwal Pegawai yang masuk pada Shift Malam tanggal : {{ date('d M Y') }} Sebanyak: <span style="font-weight:bold;">{{ $shiftmalam  }} org</span></h5> </div></div>
</div>
	      	   <a style="width:100%" onclick="loadView('{{ route('admin.malamshift.shiftmalam') }}')" class="btn  green">
                  INPUT JADWAL TGL: {{ date('d M Y') }}
                </a>
