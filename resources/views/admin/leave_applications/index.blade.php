    ...
    <div class="row">
        ...
        <div class="table-toolbar">
                    ...
                    <div class="col-md-12 form-group text-right">
                                <a class="btn green" onclick="loadView('{{route('admin.newleave.create')}}')">
                           Tambahkan Keterangan Lainnya
                                    <i class="fa fa-plus"></i> </a>
									
									               <a class="btn blue" onclick="loadView('{{route('admin.newleave_dl.create')}}')">
...
{!! HTML::script('front_assets/plugins/scrollbar/src/perfect-scrollbar.js') !!}
    ...
    <script>
      ...
      function showEdit(id, leaveType, num, pot) {
            var url = "{{ route('admin.newleave.edit',':id') }}";
            url = url.replace(':id', id);
            $.ajaxModal('#showModal', url);
            ...
            var url = "{{ route('admin.newleave_dl.edit',':id') }}";
            url = url.replace(':id', id);
            $.ajaxModal('#showModal', url);
            ...
            if (typeof id != 'undefined') {
                var url = "{{ route('admin.newleave.update',':id') }}";
                url = url.replace(':id', id);
            } else {
                url = "{{route('admin.newleave.store')}}";
            }
            $.easyAjax({
                type: 'POST',
                url: url,
