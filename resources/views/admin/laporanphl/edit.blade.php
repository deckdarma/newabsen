        ...
        <div class="col-md-12">
                ...
                <div class="portlet-body form">
                    ...
                    @else
                        <div class="table-toolbar margin-top-15">
                            <div class="row">
                                <div class="col-md-4">
                                    {!! Form::open(['route'=>["admin.laporanphl.create"], 'method' => 'GET', 'class' => "form-inline", 'id' => "new_date"]) !!}
                                    <div class="btn-group">
          ...
          @if(isset($todays_holidays_shift->date))
                            <div class="note note-warning">
								...
								{!! \Carbon\Carbon::parse($todays_holidays_shift->date)->timezone($timeZoneLocal)->format("d M Y") !!}</h3>
                                <h4>{!! trans("messages.todayIsHoliday", ["date" => $todays_holidays_shift->occassion]) !!}</h4>
                            </div>
                        @endif
		...
		@else
          @if(isset($todays_holidays->date))
			...
			@endif
						...
						@if($loggedAdmin->company->datashift==1)	
 @if(isset($todays_holidays_shift->date)) 	@else
									  <?php
...
@else
 @if(isset($todays_holidays->date)) 	@else
									  <?php
$date_jumat12 = $date->format("Y-m-d");
$timestamp12 = strtotime($date_jumat12);
$tampilkanharidata2212= date("l", $timestamp12 );
    ...
    <script>
        ...
        function loadDataTable() {
            table = $('#attendanceTable').dataTable({
                ...
                "bServerSide": true,
                "ajax": {
                    "url": "{{ URL::route("admin.laporphl.ajax_attendance") }}",
                    "data": function (d) {
...
//			 $('#halfDayLabel').show(100);
        ...
        $('#date_change').datepickerabsen({
            ...
            var url = "{{ route("admin.laporanphl.edit", "#id") }}";
...
$('.datePicker').datepickerabsen({ beforeShowDay: unavailable });
        function ajaxUpdateAttendance() {
            ...
            var url = "{{ route("admin.laporanphl.update", "#id") }}";
            url = url.replace("#id", date);
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    var route = "{{ route("admin.laporanphl.edit", "#id") }}";
